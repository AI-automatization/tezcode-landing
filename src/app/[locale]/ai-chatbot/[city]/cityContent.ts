...
const AI_CHATBOT_INTRO: Record<string, Record<ServiceLang, string>> = {
  toshkent: {
    ...
    ru: "В Ташкенте многие компании общаются с клиентами через Telegram, Instagram и WhatsApp — но операторы не могут отвечать круглосуточно. AI-чатбот работает на всех каналах 24/7, отвечает на вопросы и квалифицирует лидов.",
  ...
  samarqand: {
    ...
    ru: "В Самарканде туристы и местные клиенты часто задают вопросы через Telegram и Instagram — о ценах на отели, рестораны, музеи, часах работы. AI-чатбот сразу отвечает на все вопросы, принимает бронирования и экономит время операторов.",
...
const CITY_FAQ: Record<string, Record<ServiceLang, { q: string; a: string }>> = {
  toshkent: {
    ...
    ru: {
      q: "Сколько стоит AI-чатбот для бизнеса в Ташкенте?",
  ...
  samarqand: {
    ...
    ru: {
      q: "Подходит ли AI-чатбот для туристического бизнеса Самарканда?",
