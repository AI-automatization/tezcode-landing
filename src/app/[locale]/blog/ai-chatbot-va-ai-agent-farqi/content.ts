...
export const CONTENT: ArticleContent = {
  ...
  ru: {
    hero: {
      badge: "AI Agent / 2026",
      title: "В чём разница между AI-чат-ботом и AI-агентом? (руководство 2026)",
      subtitle:
    ...
    tldr: {
      label: "Краткий ответ",
      text:
        "Главное отличие AI-чат-бота от AI-агента — в способности действовать. Чат-бот отвечает на вопрос текстом и на этом останавливается («да, у нас есть»). AI-агент же помимо ответа выполняет практическую задачу: проверяет остаток на складе, создаёт заказ, записывает данные в CRM или 1С и передаёт сложный случай человеку. Проще говоря: чат-бот разговаривает, агент выполняет работу. Если вам нужны только вопросы-ответы на сайте — чат-бота достаточно; если нужны заказы, интеграция или реальные действия — нужен AI-агент.",
    },
    sections: [
      {
        heading: "Что такое AI-чат-бот?",
        paragraphs: [
          "AI-чат-бот — это программа, которая общается с пользователем через текст (или голос). Современный AI-чат-бот работает на базе большой языковой модели (GPT, Claude), поэтому он понимает свободный текст, а не только готовые кнопки, и отвечает естественно.",
          ...
          "Подробнее о решениях AI-чат-ботов Tezcode — на странице tezcode.dev/ai-chatbot.",
        ],
      },
      {
        heading: "Что такое AI-агент?",
        paragraphs: [
          "AI-агент — это программа, которая не просто отвечает, а доводит практическую задачу до конца ради достижения цели. Он обладает всеми возможностями чат-бота, но сверх того действует: записывает в систему, создаёт заказ, отправляет счёт, переносит данные из одной системы в другую.",
          ...
          "Чтобы узнать, как строится AI-агент и подходит ли он вашему бизнесу, прочитайте наше подробное руководство-pillar: «Создание AI-агента для бизнеса в Ташкенте» (/blog/biznes-uchun-ai-agent-yaratish). Страница услуги — tezcode.dev/ai-agent.",
        ],
      },
      {
        ...
        bullets: [
          ...
          "AI-агент: проверяет конкретный товар и остаток по складской базе, называет цену, при желании клиента оформляет заказ в CRM, спрашивает адрес и подаёт сигнал в отдел доставки.",
      ...
      },
      {
        ...
        bullets: [
          "Если ваша цель — только отвечать на частые вопросы клиентов на сайте или в Telegram, достаточно AI-чат-бота.",
          "Если агент должен проверять остаток на складе, оформлять заказ или записывать данные в CRM/1С — вам нужен AI-агент.",
      ...
      },
      {
        ...
        paragraphs: [
          "На рынке слово «AI-агент» часто используют, чтобы дороже продать обычный чат-бот. Поэтому, получая предложение, чётко спрашивайте: к каким системам подключается агент и что именно он реально делает?",
    ...
    faq: {
      ...
      items: [
        {
          q: "В чём главное отличие AI-чат-бота от AI-агента?",
          a: "Главное отличие — в способности действовать. Чат-бот отвечает на вопрос текстом и останавливается. AI-агент помимо ответа выполняет практическую задачу: проверяет остаток на складе, создаёт заказ, записывает данные в CRM или 1С и передаёт сложный случай человеку. Проще говоря: чат-бот разговаривает, агент выполняет работу.",
        },
        {
          q: "Мне достаточно чат-бота или нужен AI-агент?",
          a: "Если вам нужны только вопросы-ответы на сайте или в Telegram — достаточно чат-бота. Если агент должен реально работать со складом, заказами или CRM/1С — нужен AI-агент. Многие бизнесы начинают с чат-бота, а затем выращивают его в агента.",
        },
        {
          q: "AI-чат-бот тоже работает на базе GPT или Claude?",
          a: "Да. И современный AI-чат-бот, и AI-агент работают на базе большой языковой модели вроде GPT (OpenAI) или Claude (Anthropic). Разница не в модели, а в том, что агент подключается к внешним системам и может выполнять практические действия.",
        },
        {
          q: "AI-агент дороже чат-бота?",
        ...
        {
          q: "Можно ли потом превратить чат-бот в AI-агента?",
        ...
        {
          q: "Работает ли AI-агент на узбекском и русском?",
    ...
    cta: {
      title: "Не знаете, что нужно — чат-бот или AI-агент?",
      subtitle:
        "Команда Tezcode проанализирует ваш бизнес и вместе определит, достаточно ли вам чат-бота или нужен AI-агент. Получите бесплатную консультацию.",
