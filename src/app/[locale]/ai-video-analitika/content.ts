...
export const CONTENT: ServicePageContent = {
  ...
  ru: {
    ...
    tech: {
      ...
      items: [
        { name: "Модели компьютерного зрения", desc: "Модели обнаружения (detection) и трекинга выделяют на кадре человека, транспорт и оборудование. Точность под каждый use-case настраивается с учётом угла и освещения ваших камер." },
        { name: "Распознавание лиц и ANPR/OCR", desc: "Идентификация личности по эмбеддингам лица и OCR для чтения номерного знака. База строится на вашем списке сотрудников, клиентов или разрешённого транспорта." },
    ...
    process: {
      ...
      steps: [
        ...
        { num: "02", title: "Подключение ИИ-моделей", desc: "Подключаем модели под нужный use-case: подсчёт людей, распознавание лиц, контроль касок или ANPR. Базу (сотрудники, разрешённый транспорт) строим на ваших данных.", icon: "bot" },
    ...
    examples: {
      ...
      items: [
        ...
        { tag: "03", title: "Офис и бизнес-центр", desc: "Посещаемость и контроль доступа по лицу — без турникета и карты-пропуска. Кто и когда пришёл — фиксируется автоматически; при незнакомом лице сигнал охране." },
    ...
    pricing: {
      ...
      subtitle:
        "Разовое подключение — от $990 (≈ от 12 млн сум). Дальше вы сами управляете работой ИИ: хостинг и использование оплачиваете самостоятельно. Точная цена зависит от числа камер и задач.",
      factors: [
        { title: "Количество камер", price: "от $990 (≈ от 12 млн сум)", desc: "Сколько камер и точек анализируется — главный фактор. От одной камеры на входе до периметра целого завода; проект расширяется поэтапно." },
    ...
    faq: {
      ...
      title: "Вопросы и",
      titleAccent: "ответы",
      subtitle: "Частые вопросы об ИИ-видеоаналитике (AI).",
      items: [
        { q: "Как автоматизировать камеры с помощью ИИ?", a: "Подключив к вашим существующим IP-камерам слой ИИ (компьютерное зрение) — новое оборудование не нужно. 4 основных направления: 1) Обнаружение и подсчёт людей (посещаемость, очередь, часы пик); 2) Посещаемость и контроль доступа по лицу (без турникета); 3) Контроль охраны труда (каска/жилет, опасные зоны); 4) Распознавание автономеров (ANPR) — автоматизация шлагбаума и парковки. Подключаемся к камерам по RTSP/ONVIF, распознаём событие в реальном времени и за секунды отправляем уведомление в Telegram или дашборд. Работает on-premise (внутренний сервер) или в облаке." },
        { q: "Сколько стоит ИИ-видеоаналитика (AI)?", a: "От $990 (≈ от 12 млн сум, разовое подключение) — это единоразовая плата за подключение ИИ к вашим существующим камерам, а не подписка. После подключения вы сами управляете работой ИИ: хостинг и использование (API/сервер) оплачиваете самостоятельно. Точная цена зависит от числа камер и сложности задачи (подсчёт людей, распознавание лиц или ANPR) — уточняем в письменном предложении после бесплатной консультации и аудита камер." },
        { q: "Работает ли с существующими камерами?", a: "Да — это основная идея услуги. Покупать новые камеры или специальное оборудование не нужно. Подключаемся напрямую к вашим IP-камерам по RTSP/ONVIF (Hikvision, Dahua и другие). Слой ИИ добавляется поверх существующего видеонаблюдения, видеорегистратор остаётся на месте." },
        ...
        { q: "Кто делает ИИ-видеоаналитику (AI) в Ташкенте?", a: "Tezcode — AI Software Factory в Ташкенте и официальный резидент IT Park (свидетельство 6237). Делаем ваши существующие камеры умными с помощью ИИ: распознавание людей с ИИ, посещаемость по лицу, контроль охраны труда и распознавание автономеров. Можно прийти на бесплатную консультацию и обсудить проект: Ташкент, проспект Амира Темура, 10." },
      ],
    },
    related: {
      ...
      subtitle: "Решения Tezcode, близкие к ИИ-видеоаналитике (AI).",
      links: [
        ...
        { href: "/for-businesses", label: "Для бизнеса", desc: "На бесплатном аудите определим, где ИИ даст бизнесу наибольший эффект — продажи, безопасность или процессы." },
      ],
    },
    ...
    showcase: {
      ...
      titleAccent: "на ИИ-камере",
      subtitle: "Так ваши существующие камеры работают с ИИ (AI).",
      items: [
        {
          src: "/images/ai-video/face.jpg",
    ...
    service: {
      name: "ИИ-видеоаналитика (AI) — умные камеры на базе существующих — Tezcode",
      description:
        "ИИ-видеоаналитика (AI) в Ташкенте: делаем существующие IP-камеры умными с помощью ИИ — распознавание людей с ИИ и подсчёт, посещаемость по распознаванию лиц, контроль охраны труда (каски/СИЗ) и распознавание автономеров (ANPR). Уведомления в реальном времени, on-premise или облако. Tezcode — резидент IT Park, Узбекистан.",
      serviceType: "ИИ-видеоаналитика (AI) / внедрение системы компьютерного зрения",
    },
  },
